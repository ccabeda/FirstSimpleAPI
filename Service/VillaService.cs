...
namespace MiPrimeraAPI.Service
{
    public class VillaService : IVillaService //pasamos toda la logica del conrtoler al service
    {
        ...
        private readonly IVillaRepository _villaRepositorio; //traemos el contexto a los controladores para usar con la base de datos
        ...
        public VillaService(ILogger<VillaService> logger, IVillaRepository villaRepositorio, IMapper mapper, APIResponse apiResponse, IValidator<VillaCreateDto> villaCreateValidator,
        ...
        public async Task<APIResponse> DeleteVilla(int id)
        {
            try
            {
                ...
                var villa = await _villaRepositorio.Obtener(v => v.Id == id); //esto hace que, en la variable villa, se guarde el objeto que queremos eliminar
                                                                                        //unicamente si se encuentra en la db si no se encuentra, guarda un null
                if (villa == null)
                {
        ...
        public async Task<APIResponse> GetVilla(int id)
        {
            try //manejo de excepciones 
            {
                ...
                var villa = await _villaRepositorio.Obtener(x => x.Id == id); //que revise en la base de datos, y agarre la que ocntenga el mismo Id
                if (villa == null)
                {
        ...
        public  async Task<APIResponse> NewVilla([FromBody] VillaCreateDto CreateVillaDTO)
        {
            try
            {
                ...
                if (await _villaRepositorio.Obtener(v => v.Nombre.ToUpper() == CreateVillaDTO.Nombre.ToUpper()) != null) //buscamos en la base de datos si hay un nombre
                                                                        //igual al ingresado. si el resultado de la busqueda es !null, significa que encontro un nombre igual.
                { 
        ...
        public async Task<APIResponse> PatchVilla(int id, JsonPatchDocument<VillaUpdateDto> patchVillaDTO)
        {
            try
            {
                ...
                var villa = await _villaRepositorio.Obtener(v => v.Id == id); //guardamos el objeto que queremos actualizar o un null
                if (villa == null) //si la id no esta registrada
                {
                    _apiResponse.isExit = false;
                    _apiResponse.statusCode = HttpStatusCode.BadRequest;
                    return _apiResponse;
        ...
        public async Task<APIResponse> UpdateVilla(int id, [FromBody] VillaUpdateDto UpdateVillaDTO)
        {
            try
            {
                ...
                var villa = await _villaRepositorio.Obtener(v => v.Id == id, tracked: false); //buscamos si hay una villa con el mismo id en la db pero sin trackearla (**sin tracked:false NO FUNCIONA**)
                if (villa == null)
                {
                    _apiResponse.isExit = false;
                    _apiResponse.statusCode = HttpStatusCode.NotFound;
