...
namespace MiPrimeraAPI.Service
{
    public class NumberVillaService : INumberVillaService
    {
        ...
        private readonly INumberVillaRepository _numberVillaRepositorio; //traemos el contexto a los controladores para usar con la base de datos
        ...
        public NumberVillaService(ILogger<NumberVillaService> logger, INumberVillaRepository numberVillaRepositorio, IMapper mapper, APIResponse apiResponse)
        {
            _logger = logger;
        ...
        public async Task<APIResponse> DeleteNumberVilla(int id)
        {
            try
            {
        ...
        public async Task<APIResponse> GetNumberVilla(int id)
        {
            try //manejo de excepciones 
            {
                ...
                var numberVilla = await _numberVillaRepositorio.Obtener(x => x.VillaNo == id); //que revise en la base de datos, y agarre la que ocntenga el mismo Id
                if (numberVilla == null)
                {
        ...
        public async Task<APIResponse> NewNumberVilla([FromBody] NumberVillaCreateDto CreateNumbervillaDTO)
        {
        ...
        public async Task<APIResponse> PatchNumberVilla(int id, JsonPatchDocument<NumberVillaUpdateDto> patchNumberVillaDTO)
        {
            try
            {
                ...
                var Numbervilla = await _numberVillaRepositorio.Obtener(v => v.VillaNo == id); //guardamos el objeto que queremos actualizar o un null
                if (Numbervilla == null) //si la id no esta registrada
                {
                    _apiResponse.isExit = false;
                    _apiResponse.statusCode = HttpStatusCode.BadRequest;
                    return _apiResponse;
                }
                NumberVillaUpdateDto numberVillaDTO = _mapper.Map<NumberVillaUpdateDto>(Numbervilla); //pasamos de villa a villaDTO
                patchNumberVillaDTO.ApplyTo(numberVillaDTO); //aplicamos los cambios del json al objeto
                _mapper.Map(numberVillaDTO, Numbervilla); //mapeo los datos del dto a la villa 
                Numbervilla.FechaDeActualización = DateTime.Now;
                await _numberVillaRepositorio.Actualizar(Numbervilla); //acutalizo y guardo
                _apiResponse.statusCode = HttpStatusCode.NoContent;
                return _apiResponse;
            }
            catch (Exception ex)
        ...
        public async Task<APIResponse> UpdateNumberVilla(int id, [FromBody] NumberVillaUpdateDto UpdateNumberVillaDTO)
        {
            try
            {
                ...
                var numberVilla = await _numberVillaRepositorio.Obtener(v => v.VillaNo == id, tracked: false); //buscamos si hay una villa con el mismo id en la db pero sin trackearla (**sin tracked:false NO FUNCIONA**)
                if (numberVilla == null)
                {
                    _apiResponse.isExit = false;
                    _apiResponse.statusCode = HttpStatusCode.NotFound;
